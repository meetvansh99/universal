...
const AdminCategories = () => {
  ...
  return (
    <div className="bg-white/80 p-4 md:p-8 rounded-[1.5rem] md:rounded-3xl shadow-sm border border-white/50">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div className="flex items-center gap-2">
           <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight leading-none">Manage Categories</h2>
           <span className="bg-black text-white text-[10px] font-bold px-2 py-1 rounded-full">{categories.length}</span>
        </div>
      </div>

      {/* --- ADD FORM (Better wrapping for small screens) --- */}
      <form onSubmit={handleAdd} className="flex flex-row gap-2 mb-8">
        <input 
          ...
          value={newCategory}
          onChange={(e) => setNewCategory(e.target.value)}
          className="flex-1 min-w-0 bg-gray-50/50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold focus:outline-none focus:border-black transition-all"
        />
        <button 
          type="submit" 
          disabled={loading || !newCategory}
          className="aspect-square sm:aspect-auto bg-black text-white p-3 sm:px-6 sm:py-3 rounded-xl font-bold uppercase tracking-widest hover:bg-gray-800 disabled:opacity-50 flex items-center justify-center gap-2"
        >
          {loading ? <Loader size={18} className="animate-spin" /> : <Plus size={20} />}
      ...
      <div className="space-y-2">
        {categories.length === 0 ? (
          <p className="text-gray-400 text-center py-10 text-[10px] font-bold uppercase tracking-widest border-2 border-dashed border-gray-100 rounded-2xl">
            Empty List
          </p>
        ) : (
          categories.map((cat) => (
            <div key={cat.id} className="group flex items-center justify-between p-3 md:p-4 bg-gray-50/50 rounded-xl md:rounded-2xl border border-transparent hover:border-gray-100 transition-all hover:bg-white">
              ...
              {editingId === cat.id ? (
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <div className="flex gap-1">
                    <button onClick={handleUpdate} className="p-2 bg-green-100 text-green-600 rounded-lg"><Check size={16} /></button>
                    <button onClick={() => setEditingId(null)} className="p-2 bg-red-100 text-red-600 rounded-lg"><X size={16} /></button>
                  </div>
                </div>
              ) : (
                <>
                  <span className="font-bold text-gray-700 uppercase tracking-wide text-xs md:text-sm pl-1 truncate">
                    {cat.name}
                  </span>
                  <div className="flex gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => setEditingId(cat.id)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg"><Edit2 size={16} /></button>
